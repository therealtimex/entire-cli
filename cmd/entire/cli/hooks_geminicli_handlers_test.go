...
func TestCurrentSessionIDWithFallback_Gemini_UsesPersisted(t *testing.T) {
	tmpDir := t.TempDir()
	...
	persisted := "2024-01-02-gemini-session123"
	if err := paths.WriteCurrentSession(persisted); err != nil {
		t.Fatalf("WriteCurrentSession error: %v", err)
	}

	got := currentSessionIDWithFallback("gemini-session123")
	if got != persisted {
		t.Fatalf("currentSessionIDWithFallback() = %q, want %q", got, persisted)
	}
}

func TestCurrentSessionIDWithFallback_Gemini_FallsBackToModel(t *testing.T) {
	tmpDir := t.TempDir()
	cmd := exec.CommandContext(context.Background(), "git", "init")
	cmd.Dir = tmpDir
	if err := cmd.Run(); err != nil {
...
func TestCurrentSessionIDWithFallback_Gemini_InvalidPersistedFallsBack(t *testing.T) {
	tmpDir := t.TempDir()
	cmd := exec.CommandContext(context.Background(), "git", "init")
	cmd.Dir = tmpDir
	if err := cmd.Run(); err != nil {
	...
	invalid := "2024-01-02/malicious-gemini-path"
...
func TestCurrentSessionIDWithFallback_Gemini_MismatchedPersistedFallsBack(t *testing.T) {
	tmpDir := t.TempDir()
	cmd := exec.CommandContext(context.Background(), "git", "init")
	cmd.Dir = tmpDir
	if err := cmd.Run(); err != nil {
